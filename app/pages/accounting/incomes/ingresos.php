...
<html lang="es">
    ...
    <article class="container-ingresos">
        <section>
            <h1>INGRESOS</h1>
            <a href="addIngreso.php" class="a-button">AGREGAR INGRESO</a>
        </section>
            ...
            <table class="table table-bordered table-hover">
                <tbody>
                    ...
                    <?php
                        ...
                        $sql=$conexion->query(" SELECT * FROM ingresos JOIN accounts WHERE id_account=branch AND branch=$number ORDER BY id_ingreso DESC");
                        
                        while ($datos=$sql->fetch_object()) {
                            $value = number_format($datos->value);
                        ?>
                    <tr>
                        <td class="color-green">$ <?= $value ?></td>
                        <td><?= $datos->description ?></td>
                        <td><?= $datos->date ?></td>
                        <td>
                            <a href="editIngreso.php?id=<?=$datos->id_ingreso?>"><img src="../../../imgs/edit.png"></a>
                        </td>
                        <td>
                            <form action="controller_deleteIngreso.php?id=<?=$datos->id_ingreso?>" method="post">
